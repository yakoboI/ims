...
const initDatabase = () => {
  ...
  db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS users (
      ...
      role TEXT NOT NULL CHECK(role IN ('admin', 'storekeeper', 'sales', 'manager')),
      full_name TEXT,
...
app.get('/api/users', authenticateToken, requireRole('admin'), (req, res) => {
  db.all('SELECT id, username, email, role, full_name, created_at, is_active FROM users', (err, users) => {
    if (err) {
      return res.status(500).json({ error: sanitizeError(err) });
    }
    res.json(users);
...
app.post('/api/users', authenticateToken, requireRole('admin'), (req, res) => {
  ...
  if (!role || !['admin', 'storekeeper', 'sales', 'manager'].includes(role)) {
  ...
  db.run(
    ...
    [sanitizedUsername, sanitizedEmail, hashedPassword, role, sanitizedFullName],
    function(err) {
      if (err) {
